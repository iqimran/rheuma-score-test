...
import CdaiResults from './CdaiResults';
...
export default function CdaiCalculator({
  ...
  const calculateCDAI = (): number | null => {
    const tj = parseFloat(tenderJoints);
    const sj = parseFloat(swollenJoints);
    const pg = parseFloat(patientGlobal);
    const prg = parseFloat(providerGlobal);

    if (
      isNaN(tj) || isNaN(sj) || isNaN(pg) || isNaN(prg) ||
      tj < 0 || tj > 28 ||
      sj < 0 || sj > 28 ||
      pg < 0 || pg > 10 ||
      prg < 0 || prg > 10
    ...
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
        <div className="flex items-center gap-2 text-white">
          ...
          <h2 className="text-xl font-semibold">Calculate CDAI Score</h2>
        </div>
      </div>
      ...
      <div className="p-8">
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 mb-2 italic">
              Clinician's assessment of patient's disease activity
            </p>
            <input
              type="number"
