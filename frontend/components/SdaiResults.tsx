...
interface SdaiResultsProps {
  score: number | null;
}

export default function SdaiResults({ score }: SdaiResultsProps) {
  if (score === null) {
    return (
      <div className="mt-8 p-6 bg-gray-50 rounded-lg border-2 border-gray-200 text-center">
        <p className="text-gray-500">Enter all values to calculate SDAI score</p>
      </div>
    );
  }

  const getSeverityInfo = (sdai: number) => {
    if (sdai <= 3.3) {
      return {
        level: 'Remission',
        color: 'green',
      ...
      <div className={`p-6 rounded-xl border-2 ${severity.bgColor} ${severity.borderColor}`}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-gray-900">SDAI Score</h3>
          </div>
